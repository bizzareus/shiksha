...
/* 	public function getBlockName() {
		...
		$blockname = $this->set('blockname', $this->Filter->find('all',array('fields' => 'DISTINCT Filter.Block_name','conditions' => array('Filter.Name_Dist' => $this->data['district']))));
		}
		 */
		...
		echo json_encode($schoolname);
			...
			public function getBlockName() {
		$this->layout = null ;
		$blockname =$this->Filter->find('all',array('fields' => 'DISTINCT Filter.Block_name','conditions' => array('Filter.Name_Dist' => $this->data['district'])));
...
public function getSubject() {
 ...
 $gender = (isset($this->data['gender-type']))?$this->data['gender-type']:NULL;
...
if($case[0]=='overall-performance'){
    ...
    if(!isset($gender) and isset($district1) and !isset($block) and !isset($school) and !isset($chapter) and !isset($subject) and !isset($class)){
        //districtwise overall performance
        $comparisonparam = $district1;
    } else if (!isset($gender) and isset($district1) and isset($block) and !isset($school) and !isset($chapter) and !isset($subject) and !isset($class)){
        //blockwise overall performance
        $comparisonparam = $block;
    } else if (!isset($gender) and isset($district1) and !isset($block) and isset($school) and !isset($chapter) and !isset($subject) and !isset($class)){
        //schoolwise overall performance
        $comparisonparam = $school;
    } else if(isset($gender) and isset($district1) and !isset($block) and isset($chapter) and isset($subject) and isset($class) and !isset($school)){
		$comparisonparam = 'per-gender-district-chapter';
	} else if(isset($gender) and isset($district1) and isset($block) and isset($chapter) and isset($subject) and isset($class) and !isset($school)){
		$comparisonparam = 'per-gender-block-chapter';
	} else if(isset($gender) and isset($district1) and isset($block) and isset($chapter) and isset($subject) and isset($class) and isset($school)){
		$comparisonparam = 'per-gender-school-chapter';
	}
    // TODO: Add more cases for comparison. Exmaple: Classwise student data, subject wise, chapter wise
    /*
    if($comparisonparam == $district1){
        ...
        //Block Wise Comparision Complete
        */
		if($comparisonparam == 'per-gender-district-chapter'){
								$query_district_string = "sd.Name_Dist = '" .  $district1[0]."'";
							   ...
							   //If all parameters are set, then show a common graph
								if(count($param_query_string_complete)==5){
									$gender_score = array();
									foreach($chapter as $chapter_id){
									 $sql_for_gender = "SELECT AVG( f.Score ) 
										FROM final_results f
										INNER JOIN students s ON f.Stud_id = s.Stud_id
										INNER JOIN school_details sd ON s.School_id = sd.School_id
										WHERE (
										...
										AND (
										s.Gender =  '".$gender_char."'
										)
										AND score <> 'A' AND score <> 'N'";
										//echo $sql_for_males;
										$query1 = $this->Filter->query($sql_for_gender);
										$gender_score[$chapter_id] = ($query1[0][0]['AVG( f.Score )'])?$query1[0][0]['AVG( f.Score )']:0;
									...
									$chart_values = "";
									foreach($chapter as $chapter_id){
										$chart_values = $chart_values . ',["Chapter '.$chapter_id.'",' . $gender_score[$chapter_id]*100 . ']';
									}
									  ...
									  $chart_data = $chart_data. '
										 ...
										 $chart_options =  $chart_options.'var options5 = {
													  chart: {
														 title: "'.$gender.' Performance in All Parameters",
														subtitle: "Chapter Wise By Districts",
													  },
													  bars: "vertical" // Required for Material Bar Charts.
													};';
										...
										foreach($param_query_string_complete as $param_id){
											...
											foreach($chapter as $chapter_id){
												...
												$chart_data = $chart_data. '
												 ...
												 $chart_options =  $chart_options.'var options'.$param_id.' = {
															  chart: {
																 title: "'.$gender.' Performance in '.get_param_name($param_id).'",
																subtitle: "Chapter Wise for Districts",
															  },
															  bars: "vertical" // Required for Material Bar Charts.
															};';
		...
		if($comparisonparam == 'per-gender-block-chapter'){
								$query_block_string = "sd.Block_name = '" .  $block[0]."'";
